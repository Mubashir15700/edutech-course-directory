...
export default function Profile() {
    ...
    // Quick structural calculation arrays
    // const completedCoursesCount = user.enrolledCourses ? user.enrolledCourses.filter(c => c.progress === 100).length : 0;

    return (
        <div className="min-h-[calc(100vh-64px)] bg-gray-50/50 p-4 sm:p-6 lg:p-8">
            <div className="max-w-5xl mx-auto">

                {/* 1. Profile Top Overview Header */}
                <div className="bg-white rounded-2xl border border-gray-200/80 p-6 shadow-sm mb-8 flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left">
                    ...
                    <div className="flex-grow">
                        ...
                        <p className="text-xs text-gray-400 mt-2">
                            Studying since {new Date(user.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'long' })}
                        </p>
                    </div>
                ...
                {/* 2. Primary Layout Switching Logic */}
                {activeTab === "courses" ? (
                                        ...
                                        <div key={course._id} className="bg-white border border-gray-200/80 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition duration-200 flex flex-col">
                                            ...
                                            <div className="p-5 flex-grow flex flex-col justify-between">
                                                <div>
                                                    ...
                                                    <p className="text-xs text-gray-400 font-medium mb-4">By {course.instructor}</p>
                                                ...
                                                <div className="pt-3 border-t border-gray-50 mt-auto flex flex-col gap-3">
                                                    <div className="text-xs text-gray-500 truncate">
                                                        <span className="font-bold text-gray-400 block uppercase text-[9px] tracking-wider">Up Next</span>
                                                        {/* <span className="font-medium text-gray-700">{course.nextLesson}</span> */}
                                                    </div>
                                                    <Link
                    ...
                    /* 3. Basic Security Profile Account Settings Subsection Container */
                    <div className="bg-white border border-gray-200/80 rounded-2xl p-6 shadow-sm max-w-xl">
                        ...
                        <form className="space-y-4" onSubmit={e => e.preventDefault()}>
                            <div>
                                ...
                                <input type="text" defaultValue={user.name} className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-gray-700" />
                            </div>
                            <div>
                            ...
                            <div className="pt-2 flex justify-end">
                                <button className="px-4 py-2.5 bg-blue-600 text-white rounded-xl font-semibold text-xs uppercase tracking-wider hover:bg-blue-700 transition shadow-sm">
                                    Save System Profile
                                </button>
                            </div>
                        </form>
